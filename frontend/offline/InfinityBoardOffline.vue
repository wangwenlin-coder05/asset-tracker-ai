<template>
  <div class="ib-root" :class="{ 'ib-dark': darkMode, 'ib-focus-mode': focusMode }" @contextmenu.prevent="onContextMenu">
      ...
      <div class="ib-canvas" :class="{ 'no-transition': isPanning || isWheelPanning }"
        ...
        <template v-for="el in visibleElements" :key="el.id">
          ...
          <div v-else-if="el.type === 'text' && editingId !== el.id" class="ib-element ib-text"
            ...
            :style="{ left: el.x + 'px', top: el.y + 'px', width: el.width + 'px', height: el.height + 'px', fontSize: el.size + 'px', color: el.color || '#1e293b', fontWeight: el.fontWeight || 400, fontStyle: el.fontStyle || 'normal', lineHeight: el.lineHeight || 1.5 }"
    ...
    <!-- 文字编辑固定菜单栏（屏幕下方，编辑文字时显示） -->
    <div v-if="editingEl && (editingEl.type === 'text' || editingEl.type === 'note')" class="ib-editbar">
      <span class="ib-editbar-label">字号</span>
      <button class="ib-editbar-btn" title="减小字号" @mousedown.prevent.stop="stepFontSize(-2)" @touchstart.prevent.stop="stepFontSize(-2)">A−</button>
      <span class="ib-editbar-val">{{ editingEl.size || 16 }}</span>
      <button class="ib-editbar-btn" title="增大字号" @mousedown.prevent.stop="stepFontSize(2)" @touchstart.prevent.stop="stepFontSize(2)">A+</button>
      <span class="ib-editbar-sep"></span>
      <button class="ib-editbar-btn" :class="{ on: editingEl.fontWeight === 700 }" title="加粗"
        @mousedown.prevent.stop="toggleFontProp('fontWeight')" @touchstart.prevent.stop="toggleFontProp('fontWeight')"><b>B</b></button>
      <button class="ib-editbar-btn" :class="{ on: editingEl.fontStyle === 'italic' }" title="斜体"
        @mousedown.prevent.stop="toggleFontProp('fontStyle')" @touchstart.prevent.stop="toggleFontProp('fontStyle')"><i>I</i></button>
      <span class="ib-editbar-sep"></span>
      <label class="ib-editbar-color" title="文字颜色">
        <input type="color" :value="editingEl.color || '#1f2329'"
          @input.prevent.stop="editingEl.color = $event.target.value; scheduleSave()" />
      </label>
      <span class="ib-editbar-sep"></span>
      <span class="ib-editbar-label">行高</span>
      <button class="ib-editbar-btn" title="减小行高" @mousedown.prevent.stop="stepLineHeight(-0.1)" @touchstart.prevent.stop="stepLineHeight(-0.1)">↕−</button>
      <span class="ib-editbar-val">{{ (editingEl.lineHeight || 1.5).toFixed(1) }}</span>
      <button class="ib-editbar-btn" title="增大行高" @mousedown.prevent.stop="stepLineHeight(0.1)" @touchstart.prevent.stop="stepLineHeight(0.1)">↕+</button>
      <span class="ib-editbar-sep"></span>
      <button class="ib-editbar-done" title="完成编辑" @mousedown.prevent.stop="finishEdit()" @touchstart.prevent.stop="finishEdit()">✓ 完成</button>
    </div>
...
const editingEl = computed(() => {
  ...
  return elements.value.find(e => e.id === editingId.value) || null
})
...
function stepFontSize(delta) {
  ...
  if (!el) return
  pushHistory()
  el.size = Math.max(10, Math.min(200, (el.size || 16) + delta))
  scheduleSave()
}

function toggleFontProp(prop) {
  const el = editingEl.value
  if (!el) return
  pushHistory()
  if (prop === 'fontWeight') {
...
function stepLineHeight(delta) {
  ...
  if (!el) return
  pushHistory()
  el.lineHeight = Math.max(1, Math.min(3, Math.round(((el.lineHeight || 1.5) + delta) * 10) / 10))
  scheduleSave()
}
...
// ==================== 触摸支持（平板） ====================
// 状态：touchState = { mode, ... }
// mode: 'pan' 单指平移画布 | 'pinch' 双指缩放 | 'drag' 拖动元素 | null 无
let touchState = null
let touchDbl = { t: 0, x: 0, y: 0, id: null }  // 双击检测
const TOUCH_DRAG_THRESHOLD = 6

function onTouchStart(e) {
  // 编辑中：触摸输入框内部时不处理
  if (e.target.closest('.ib-text-input') || e.target.closest('.ib-note-textarea')) return
  resetIdleTimer()
  const rect = viewportRef.value?.getBoundingClientRect()
  if (!rect) return
  ...
  if (e.touches.length === 2) {
    // 双指捏合缩放
    touchState = { mode: 'pinch', d: Math.hypot(e.touches[0].clientX - e.touches[1].clientX, e.touches[0].clientY - e.touches[1].clientY), z0: zoom.value, cx: (e.touches[0].clientX + e.touches[1].clientX) / 2, cy: (e.touches[0].clientY + e.touches[1].clientY) / 2, vx0: viewportX.value, vy0: viewportY.value }
    return
  }
  ...
  if (e.touches.length === 1) {
    ...
    // 双击检测：同一位置 300ms 内再次触摸
    const now = Date.now()
    if (now - touchDbl.t < 300 && Math.abs(cx - touchDbl.x) < 20 && Math.abs(cy - touchDbl.y) < 20) {
      touchDbl.t = 0
      handleTouchDblTap(cx, cy, e)
      return
    }
    touchDbl = { t: now, x: cx, y: cy, id: null }

    // 命中元素 → 拖动
    const elEl = e.target.closest('.ib-element')
    if (elEl) {
      const id = elEl.dataset ? elEl.dataset.elId || findElIdByEl(elEl) : findElIdByEl(elEl)
      const obj = elements.value.find(o => o.id === id)
      if (obj && !obj.locked) {
        // 取消当前选择，选中该元素
        if (editingId.value && editingId.value !== obj.id) finishEdit()
        if (!selectedIds.value.includes(obj.id)) selectedIds.value = [obj.id]
        const p = screenToCanvas(t.clientX, t.clientY)
        touchState = { mode: 'drag', id: obj.id, sx: t.clientX, sy: t.clientY, ox: obj.x, oy: obj.y, mx: p.x, my: p.y, moved: false, el: elEl }
        if (elEl.dataset) elEl.dataset.touchDragging = '1'
        return
      }
    }

    // 空白 → 平移画布
    touchState = { mode: 'pan', sx: t.clientX, sy: t.clientY, vx0: viewportX.value, vy0: viewportY.value }
  }
}

function findElIdByEl(el) {
  // Vue 渲染的元素通过 :data-el-id 或 DOM 结构关联；无 data-el-id 时取最近关联
  return el.dataset.elId || null
}
...
function onTouchMove(e) {
  ...
  if (touchState.mode === 'pinch' && e.touches.length === 2) {
    ...
    const newZoom = Math.min(2, Math.max(0.3, touchState.z0 * d / touchState.d))
    const ratio = newZoom / touchState.z0
    // 以双指中心缩放
    const cx = (e.touches[0].clientX + e.touches[1].clientX) / 2
    const cy = (e.touches[0].clientY + e.touches[1].clientY) / 2
    const rect = viewportRef.value?.getBoundingClientRect()
    const rx = cx - rect.left, ry = cy - rect.top
    viewportX.value = rx - (rx - touchState.vx0) * ratio
    viewportY.value = ry - (ry - touchState.vy0) * ratio
    zoom.value = newZoom
  } else if (touchState.mode === 'pan' && e.touches.length === 1) {
    viewportX.value = touchState.vx0 + (e.touches[0].clientX - touchState.sx)
    viewportY.value = touchState.vy0 + (e.touches[0].clientY - touchState.sy)
  } else if (touchState.mode === 'drag' && e.touches.length === 1) {
    const dx = e.touches[0].clientX - touchState.sx
    const dy = e.touches[0].clientY - touchState.sy
    if (Math.abs(dx) > TOUCH_DRAG_THRESHOLD || Math.abs(dy) > TOUCH_DRAG_THRESHOLD) touchState.moved = true
    if (touchState.moved) {
      const obj = elements.value.find(o => o.id === touchState.id)
      if (obj) {
        obj.x = touchState.ox + dx / zoom.value
        obj.y = touchState.oy + dy / zoom.value
      }
    }
  }
}
...
function onTouchEnd() {
  if (touchState && touchState.mode === 'drag' && touchState.moved) {
    pushHistory()
    scheduleSave()
  }
...
// 触摸双击：命中元素→编辑，空白→按当前工具创建元素
function handleTouchDblTap(cx, cy, e) {
  const elEl = e.target.closest('.ib-element')
  if (elEl) {
    const id = elEl.dataset ? (elEl.dataset.elId || findElIdByEl(elEl)) : null
    const obj = id ? elements.value.find(o => o.id === id) : null
    if (obj) {
      startEditText(obj)
      return
    }
  ...
  if (connectingFrom.value) return
  if (activeTool.value === 'note' || activeTool.value === 'text') {
    addElement(activeTool.value, pos.x - (activeTool.value === 'note' ? 80 : 0), pos.y - (activeTool.value === 'note' ? 50 : 0))
    const newEl = elements.value[elements.value.length - 1]
    editingId.value = newEl.id
    startEditText._savedText = ''
    nextTick(() => {
      if (newEl.type === 'text' && editingInput.value) editingInput.value.focus()
      else if (editingNoteArea.value) editingNoteArea.value.focus()
    })
  } else {
    addElement('note', pos.x - 80, pos.y - 50)
    const newEl = elements.value[elements.value.length - 1]
    editingId.value = newEl.id
    startEditText._savedText = ''
    nextTick(() => { if (editingNoteArea.value) editingNoteArea.value.focus() })
  }
  activeTool.value = 'pan'
}
...
.ib-editbar-color input[type="color"] {
  position: absolute;
  inset: -8px;
  width: calc(100% + 16px);
